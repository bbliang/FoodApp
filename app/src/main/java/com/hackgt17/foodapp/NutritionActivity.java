...
public class NutritionActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        food_item = (Ingredient) getIntent().getSerializableExtra("ingredient");

        chart = (BarChart) findViewById(R.id.chart);
        prepareChartData(food_item);
    }

    public void prepareChartData(Ingredient food){
        entries = new ArrayList<BarEntry>();
        DecimalFormat df = new DecimalFormat();
        df.setMaximumFractionDigits(2);

        entries.add(new BarEntry(0, (float) (food.getCalories()/2000)*100));
        entries.add(new BarEntry(1, (float) (food.getTotalFat()/65)*100));
        ...
        entries.add(new BarEntry(5, (float) food.getSodium()));
